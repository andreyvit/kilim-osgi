...
public class MethodFlow extends MethodNode {
	...
	/**
	 * The list of basic blocks, in the order in which they occur in the class
	 * file. Maintaining this order is important, because we'll use it to drive
	 * duplication (in case of JSRs) and also while writing out the class file.
	 */
	...
	public void verifyPausables() throws KilimException {
		...
		if (!hasPausableAnnotation && !pausableMethods.isEmpty()) {
			String msg;
			String name = toString(classFlow.getClassName(), this.name,
					this.desc);
	...
	private void checkStatus(String superClassName, String methodName,
		...
		if (status == Detector.METHOD_NOT_PAUSABLE && hasPausableAnnotation) {
			throw new KilimException(
					"Base class method is not pausable, but derived class is: "
							+ "\nBase class = " + superClassName
							+ "\nDerived class = " + this.classFlow.name
							+ "\nMethod = " + methodName + desc);
		}
	...
	public void visitMethodInsn(int opcode, String owner, String name,
		...
		MethodInsnNode min = (MethodInsnNode) instructions.get(instructions
				.size() - 1);
		allMethods.add(min);
	}

	public void postProcess() {
		for (MethodInsnNode min : allMethods)
			checkPausable(min);
		allMethods = null;
	}
...
//		System.out.println("</MethodFlow.checkPausabilityOf()>");
	...
	private void assignCatchHandlers() {
		...
		for (int i = 0; i < tcbs.size(); i++) {
			...
			handlers.add(new Handler(getLabelPosition(tcb.start),
					getLabelPosition(tcb.end) - 1, // end is inclusive
					tcb.type, getOrCreateBasicBlock(tcb.handler)));
		}
		for (BasicBlock bb : basicBlocks) {
	 ...
	 * vars are live, mixes it with its own uses and defs and passes on a new
	 * list of live vars to its predecessors. Since the information bubbles up
	 * the chain, we iterate the list in reverse order, for efficiency. We could
	 * order the list topologically or do a depth-first spanning tree, but it
	 * seems like overkill for most bytecode procedures. The order of
	 * computation doesn't affect the correctness; it merely changes the number
	 * of iterations to reach a fixpoint.
	 */
	 ...
	 * encounter a label. We don't really know until we are done with that pass
	 * whether a label is the target of a branch instruction or it is there
	 * because of an exception handler. See coalesceWithFollowingBlock() for
	 * more detail.
	 */
	private void consolidateBasicBlocks() {
		...
		for (BasicBlock bb : basicBlocks) {
			if (!bb.hasFlag(COALESCED)) {
				...
				// The original bb's followers should have been marked as
				// processed.
				bb.setId(pos++);
				newBBs.add(bb);
			}
	...
	private boolean checkNoBasicBlockLeftBehind() { // like
		...
		for (BasicBlock bb : bbs) {
			...
			for (BasicBlock succ : bb.successors) {
				assert succ.isInitialized() : "Basic block not inited. Succ of "
						+ bb;
	...
	private void dataFlow() {
		workset = new PriorityQueue<BasicBlock>(instructions.size(),
				new BBComparator());
	 ...
	 * Unlike MethodNode, we don't keep LabelNodes inline, so we need to do
	 * visitLabel ourselves.
	 * 
	...
	public static void acceptAnnotation(final AnnotationVisitor av,
